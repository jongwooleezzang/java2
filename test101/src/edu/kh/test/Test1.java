package edu.kh.test;
//문제1. 컬렉션 List를 이용해 IP별 사용자를 관리하고, IP를 입력 받아 일치하는 사용자가 있는지 검색하는 코드를 작성하였으나 
//오류 발생 및 정상 수행되지 않았다
...
public class Test1 {
	...
	public static void main(String[] args) {
		...
		List<IPInfo> ipInfoList = new ArrayList<IPInfo>(); //List<IPInfo> ipInfoList = null;	
		//ipInfoList는 ArrayList로 생성해야 하지만, null로 대입하였으므로 ipInfoList에 요소를 추가(add)하는 13~15 문장은 NullPointerException이 발생 
		...
		for(int i=0 ; i<ipInfoList.size() ; i++) {
			if(ipInfoList.get(i).getIp().equals(ip)) { //if(ipInfoList.get(i) == ip) { //ipInfoList.get(i)는 특정 번째의 IPInfo 클래스의 객체이고, ip 는 String 변수 이므로, 
																						//특정 객체가아닌 ip의 일치를 확인해야하므로			    , String은 == 가아닌 .equals() 을 사용해야하므로
